public class constructTreeFromPostAndIn{
  ...
  private static TreeNode buildTreeUtil(int[] in, int[] post, int inStrt, int inEnd){
    ...
    root.right = buildTreeUtil(in, post, iIndex+1, inEnd);
    root.left = buildTreeUtil(in, post, inStrt, iIndex-1);
